...
Usage:
  ...
  python fink_download_full_cutouts.py --obj_id 170032915988086813 --mjd_min 60800.0
  python fink_download_full_cutouts.py --obj_id 170032915988086813 --mjd_min 60800.0 --mjd_max 60900.0
...
def fetch_single_cutout(dia_source_id: int, kind: str) -> np.ndarray | None:
    ...
    -------
    np.ndarray or None
        Array of shape (H, W) float32, or None on failure.
    """
    r = requests.get(
        f"{FINK_API}/cutouts",
...
def fetch_all_cutouts(dia_source_id: int) -> dict[str, np.ndarray] | None:
    ...
    dict[str, np.ndarray] or None
        {'Science': arr, 'Template': arr, 'Difference': arr},
...
def download_full_cutouts(
    ...
    3. For each retained diaSource fetch the 3 stamps  →  cutouts/*.npy
    ...
    outdir : Path, optional
        Output directory. Defaults to ./fullcutouts_{dia_object_id}/
    skip_existing : bool
        If True, skip diaSources whose .npy files already exist on disk.
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Only download diaSources with midpointMjdTai >= MJD_MIN (e.g. 60800.0)",
    )
    parser.add_argument(
        "--mjd_max",
        type=float,
        default=None,
        help="Only download diaSources with midpointMjdTai <= MJD_MAX (e.g. 60900.0)",
    )
